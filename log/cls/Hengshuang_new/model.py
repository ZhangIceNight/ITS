...
class Backbone(nn.Module):
    def __init__(self, cfg):
        ...
        self.transformer1 = PointTransformerLayer(
            dim = 32,
        ...
        for i in range(nblocks):
            ...
            self.transformers.append(PointTransformerLayer(
                                        dim = channel,
                                        pos_mlp_hidden_dim = 64,
                                        attn_mlp_hidden_mult = 4,
                                        num_neighbors = 16          # only the 16 nearest neighbors would be attended to for each point
    ...
    def forward(self, x):
        xyz = x[..., :3]
        points = self.transformer1(pos = xyz, x = self.fc1(x))
        xyz_and_feats = [(xyz, points)]
        for i in range(self.nblocks):
            xyz, points = self.transition_downs[i](xyz, points)
            points = self.transformers[i](pos = xyz, x = points) # shape bubian
...
if __name__ == "__main__":
    x = torch.randn(4,200,200)
    pos = torch.randn(4,200,3)
    model = PointTransformerLayer(dim = 200,
